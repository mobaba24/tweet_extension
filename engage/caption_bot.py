...
async def on_pick(update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await ctx.bot.send_message(
        chat_id,
        "📋 کپشن آماده‌ست:\n\n" + cap +
        "\n\n📲 ساده‌ترین راه: روی عکس بالا بزن ← آیکن «اشتراک‌گذاری» ← اینستاگرام یا ایکس رو انتخاب کن "
        "(عکس مستقیم منتقل می‌شه، بعد کپشن رو پیست کن).\n\n"
        "یا دستی:\n"
        "۱) عکس رو ذخیره کن 💾\n"
        "۲) ایکس: دکمه‌ی پایین متن رو آماده می‌کنه، عکس رو ضمیمه کن.\n"
